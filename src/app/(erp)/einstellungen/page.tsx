...
async function saveBenachrichtigungen(formData: FormData) {
  'use server'
...
export default async function EinstellungenPage() {
  ...
  const schalterStand = await benachrichtigungsSchalter(sql)
  const meldungen = await letzteBenachrichtigungen(sql, 8)
  ...
  return (
    <>
      ...
      <Card title="Benachrichtigungen (Telegram)">
        <div className="mono-label" style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 10 }}>
          <span className={telegram ? 'led ok' : 'led off'} />
          {telegram
            ? 'Telegram verbunden (TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID gesetzt)'
            : 'nicht konfiguriert — TELEGRAM_BOT_TOKEN und TELEGRAM_CHAT_ID als Umgebungsvariablen setzen'}
        </div>
        <ActionForm action={saveBenachrichtigungen}>
          <div style={{ marginBottom: 10 }}>
            {(Object.keys(SCHALTER_LABELS) as (keyof typeof SCHALTER_LABELS)[]).map((k) => (
              <label key={k} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                <input type="checkbox" name={k} defaultChecked={schalterStand[k]} />
        ...
        <p className="small muted" style={{ margin: '8px 0 0' }}>
          Bot beim @BotFather anlegen, Token als <span className="mono">TELEGRAM_BOT_TOKEN</span> setzen,
          dem Bot einmal schreiben (oder in die Gruppe holen), dann „Chat-IDs ermitteln" und die ID als{' '}
          <span className="mono">TELEGRAM_CHAT_ID</span> setzen. Gesendet wird jede Minute vom Cron
          „jobs"; Anmeldungen zusätzlich sofort. Fehlversuche werden je Konto und Viertelstunde gebündelt.
        </p>
        {meldungen.length > 0 && (
          <TableWrap>
