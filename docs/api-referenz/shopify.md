# Shopify: API-Referenz für den Nachbau
...
## 3. Fulfillments mit Tracking (die Versand-Rückmeldung)

Shopify nutzt das **FulfillmentOrder-Modell**: Order → 1..n FulfillmentOrder (Line Items je Location, mit `supportedActions` und `remainingQuantity`) → 1..n Fulfillment. Apps fulfillen nie die Order direkt.

Ablauf „DHL-Label erstellt → Tracking in Shopify + Versandmail an den Kunden":

1. FulfillmentOrders der Order abfragen (prüfen: `supportedActions` enthält `CREATE_FULFILLMENT`).
2. **`fulfillmentCreate`**:
...
mutation {
  fulfillmentCreate(fulfillment: {
    lineItemsByFulfillmentOrder: [{
      ...
      # ohne fulfillmentOrderLineItems => Voll-Fulfillment; mit Angabe => Teil-Fulfillment
    }]
    trackingInfo: { company: "DHL", number: "00340434…", url: "https://www.dhl.de/…" }
    notifyCustomer: true   # Default ist false! true löst Shopifys Versandbestätigungs-Mail aus
  }) {
...
```
   ...
   Bekannte `company`-Werte (z. B. „DHL") erzeugen automatisch Tracking-URLs.
3. Nachträgliche Korrekturen: **`fulfillmentTrackingInfoUpdate`** (`fulfillmentId`, `trackingInfoInput { company, number, numbers, url, urls }`, `notifyCustomer`). Mehrere Pakete je Fulfillment über `numbers`/`urls` (Multicollo).

**Abzufangende Fehlerbilder:** `nonFulfillableQuantity > 0` (Out-of-Stock blockiert Fulfillment), Produkte ohne Location-Zuordnung (422), Rate-Limits (Retry mit Backoff).

## 4. Tags auf Orders setzen (optional)

Bevorzugt **`tagsAdd`** (additiv, überschreibt bestehende Tags nicht); Gegenstück `tagsRemove`. Scope: `write_orders`. `orderUpdate` mit `tags` ersetzt dagegen die **komplette** Tag-Liste — für additive Tags `tagsAdd` verwenden.

```graphql
mutation {
  tagsAdd(id: "gid://shopify/Order/1234567890", tags: ["in-fertigung"]) {
    node { id }
    userErrors { field message }
  }
}
...
## 5. Relevante Order-Datenfelder (GraphQL `Order`)
