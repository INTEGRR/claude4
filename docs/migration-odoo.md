...
| 8 | Abschluss | Nummernkreise hochziehen — `sequences.next_number` UND per `setval` die PG-Sequenz `seq_<code>`, aus der `next_sequence()` seit 0026 tatsächlich zieht (Entscheidungslog 2026-08-27; Reparatur bestehender Instanzen: Migration 0076) —, MO-Präfix auf `WH/MO/`, `refresh_analytics()`, **`datenTuev()` als harte Abnahme** (Befund = Exit ≠ 0, prüft seither auch „Nummernkreise vor dem Belegbestand"). |
